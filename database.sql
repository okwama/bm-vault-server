...
CREATE TABLE IF NOT EXISTS requests (
  ...
  FOREIGN KEY (atm_id) REFERENCES atms(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS teams (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (crew_commander_id) REFERENCES staff(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS team_members (
  ...
  UNIQUE KEY unique_team_member (team_id, staff_id)
);
...
CREATE TABLE IF NOT EXISTS vault_update (
  id INT PRIMARY KEY AUTO_INCREMENT,
...
INSERT INTO vault_users (username, email, password, role) VALUES 
...
INSERT INTO vault (id, name, current_balance) VALUES 
(1, 'Vault 1', 0.00)
ON DUPLICATE KEY UPDATE id=id; 